...
import { rootURL, gateWay, isMock, ServerCode, ServerCodeMap, RedirectMap } from './config';
import { RequestConfig, ResponseConfig, ResponseError } from './interface';

const service: AxiosInstance = axios.create({
    baseURL: rootURL + gateWay, // api的base_url
...
const errHandle: <R>(err: AxiosError<R> | ResponseError<R>) => Promise<R> = err => {
    ...
    const ERR_MESSAGE = ServerCodeMap[code] || '接口异常';
    // 若code是400，则不会弹出message
    code !== ServerCode.CONTINUE ? message.error(ERR_MESSAGE) : console.error({ ...err, message: ERR_MESSAGE });
    if (isResponseError(err)) {
        errResult = err.data;
    }
